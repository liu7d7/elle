...
struct pmh_tables {
  ...
  constexpr std::size_t lookup(const KeyType & key, const HasherType& hasher) const {
    ...
    if (!d.is_seed()) { return static_cast<std::size_t>(d.m_val()); } // this is narrowing std::uint64 -> std::size_t but should be fine
    else { return second_table_[hasher(key, static_cast<std::size_t>(d.m_val())) % M]; }
  }
};
...
pmh_tables<M, Hash> constexpr make_pmh_tables(const carray<Item, N> &
  ...
  for (const auto & bucket : buckets) {
    ...
    if (bsize == 1) {
      ...
      G[bucket.m_hash] = {false, static_cast<std::uint64_t>(bucket[0])};
    } else if (bsize > 1) {
      ...
      // Put successful seed in G, and put size to items in their slots
      // assert(bucket.hash == hash(key(items[bucket[0]]), step_one.seed) % M);
      G[bucket.m_hash] = d;
